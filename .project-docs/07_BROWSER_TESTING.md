...
- T-code 칩 바 노출 또는 해당 없음 확인
- `data-glossary` 툴팁 동작
- 시뮬레이션/탭/퀴즈/버튼 등 주요 인터랙션 동작
- 새 입력형 인터랙션은 정상 입력 1회, 빈값/오답 1회, reset/retry 1회를 확인
- 모바일 폭에서 텍스트 겹침 없음
...
| 글로서리/T-code만 | 해당 Lesson 칩 바 또는 해당 없음, 툴팁, `used_in_lessons` 반영. T-code 지도는 구조를 바꾼 경우만 확인 |
...
반복 검증이나 스크린샷이 필요할 때만 로컬 scratch에 Playwright를 설치해 사용한다. 저장소 루트에서 `npm init`을 실행하지 않는다. 일반 Lesson 검증은 브라우저 수동 확인과 콘솔 체크로 충분하다.

```powershell
$scratch = "C:\ui5\scratch\sapui5-playwright"
