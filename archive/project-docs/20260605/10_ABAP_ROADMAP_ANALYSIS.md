...
## 5. 2026-06-02 샘플 UI 검토 결과

### 5-1. 검토한 화면 방향

2026-06-02 작업에서는 신규 ABAP 커리큘럼을 실제 웹 화면으로 전환하기 위한 여러 HTML 샘플을 비교했다. 핵심 요구는 단순 카드형 목록이 아니라, 학습자가 긴 커리큘럼을 빠르게 탐색하고 현재 위치를 잃지 않도록 돕는 구조였다.

주요 후보 구조는 다음과 같다.

- 상단 Track 탭으로 `Track 1` 이론 흐름과 `Track 2` 실무 응용 흐름을 분리한다.
- 좌측에는 해당 Track의 THEORY 목록을 세로 리스트로 출력한다.
- 좌측 THEORY를 선택하면 본문에는 선택한 THEORY 하나만 출력한다.
- 하위 학습 단위는 Stage 7 Navigation 패널 안의 학습 목차로 보여준다.
- 학습 목차는 트리 구조와 Scroll Spy를 사용해 현재 읽는 단위를 표시한다.
- 본문 안의 핵심 용어는 팝업으로 설명할 수 있게 한다.
- 데이터는 가능하면 JSON 또는 JS 데이터 파일로 분리해 HTML 구조 수정 없이 커리큘럼을 갱신할 수 있게 한다.

### 5-2. 샘플별 장점

| 샘플 계열 | 장점 | 반영 방향 |
|---|---|---|
| Antigravity | 가장 깔끔한 본문 디자인, 안정적인 시각 위계, 여백과 카드 밀도 균형 | v7 sampleA의 기본 디자인 방향으로 채택 |
| Claude | 현재 THEORY 유지, 용어 팝업, 우측 Navigation, 상세 설명, 공통 디자인 | Navigation 학습 목차와 용어 팝업 기능으로 반영 |
| Codex | 검색 기능, JSON 데이터 기반 수정, 상세 설명, 공통 디자인 | 데이터 분리와 검색/필터 기능으로 반영 |

### 5-3. 현재 선호안

현재 선호안은 `docs/roadmap/abap-curriculum-codex-v7_sampleA-20260602-165628.html`이다.

이 샘플은 다음 판단을 반영한다.

- 모든 THEORY를 본문에 한 번에 나열하지 않는다.
- 좌측 THEORY 클릭 시 본문을 해당 THEORY 상세로 교체한다.
- 본문 우측의 별도 학습 목차 패널은 제거하고, Stage 7의 Navigation 패널 안에 학습 목차를 넣는다.
- 본문 폭을 넓혀 상세 설명, 실습 설계, 주의점, 용어 설명을 읽기 쉽게 만든다.
- Track 탭은 스크롤 중 유지하되 본문을 가리지 않도록 검색/필터와 분리한다.
- 좌측 목록 스크롤은 본문 스크롤로 전파되지 않도록 한다.
- 학습 목차 클릭과 Scroll Spy는 현재 하위 학습 단위를 따라가도록 한다.

### 5-4. 운영 반영 전 남은 결정

1. 최종 샘플을 하나로 확정한다.
2. `docs/roadmap/abap-curriculum-data.js` 또는 별도 JSON 파일을 최종 데이터 소스로 확정한다.
3. GitHub Pages에서 정적 HTML이 데이터 파일을 안정적으로 읽는 방식을 확정한다.
4. `pages/roadmap.html` 또는 `pages/abap.html`에 샘플을 운영 링크로 노출할지 결정한다.
5. 샘플 파일 중 보존할 버전과 archive로 이동할 버전을 정리한다.
6. 최종 운영본 파일명을 샘플명에서 운영명으로 전환할지 결정한다.
