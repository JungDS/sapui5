    ...
    for tag in soup.find_all(True):
        if tag.get('class'):
            for c in tag.get('class'):
                if 'code' in c or 'tour' in c or 'abap' in c or 'shiki' in c or 'editor' in c:
                    classes.add(c)
...
files = glob.glob(os.path.join(base_dir, "code-learning", "*.html"))

print("=== Phase 3 (Code Learning) 파일 콘텐츠 및 컴포넌트 분석 ===")
all_components = set()
for f in files:
    res = analyze_html(f)
