    ...
    for tag in soup.find_all(True):
        if tag.get('class'):
            for c in tag.get('class'):
                if 'quiz' in c or 'card' in c or 'mission' in c or 'capstone' in c or 'drag' in c:
                    classes.add(c)
...
files = glob.glob(os.path.join(base_dir, "quizzes", "*.html")) + glob.glob(os.path.join(base_dir, "capstone", "*.html"))

print("=== Phase 5 (Quizzes & Capstone) 파일 콘텐츠 및 컴포넌트 분석 ===")
all_components = set()
for f in files:
    res = analyze_html(f)
