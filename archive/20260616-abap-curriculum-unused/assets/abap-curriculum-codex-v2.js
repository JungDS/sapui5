(function () {
  ...
  function renderStats(data) {
    ...
    return `
      <div class="curv2-stats">
        <div class="curv2-stat"><strong>${stats.tracks}</strong><span>Tracks</span></div>
        <div class="curv2-stat"><strong>${stats.sections}</strong><span>Sections</span></div>
        <div class="curv2-stat"><strong>${stats.units}</strong><span>Learning Units</span></div>
        <div class="curv2-stat"><strong>${formatHours(stats.hours)}</strong><span>Recommended Hours</span></div>
      </div>
    `;
  }
  ...
  function initPremium(root, data, termIndex) {
      ...
      <div class="curv2-premium-wrap">
        <section class="curv2-premium-hero">
          <div>
            ...
            <div class="curv2-kicker">ABAP Curriculum Codex v2</div>
    ...
    `;
    wireMasterDetail(root, data, termIndex, state, { mode: "premium" });
  }
  ...
  function wireMasterDetail(root, data, termIndex, state, options) {
    ...
    const render = ({ keepHash = false } = {}) => {
      ...
      renderDetail(els.detail, section, termIndex, options.mode);
  ...
  function renderTabs(container, data, state) {
    container.innerHTML = data.tracks.map((track, index) => {
      ...
      return `<button class="curv2-tab" type="button" data-track-index="${index}" aria-selected="${index === state.trackIndex ? "true" : "false"}">${escapeHtml(label)} · ${escapeHtml(track.track_name)}</button>`;
    }).join("");
  }
  ...
  function renderDetail(container, section, termIndex, mode) {
    if (!section) {
    ...
    container.innerHTML = `
      ...
      ${units.map((unit, index) => renderUnit(unit, index, mode)).join("")}
    `;
  }

  function renderUnit(unit, index, mode) {
  ...
  function renderToc(container, section) {
    ...
    container.innerHTML = `
      ...
      ${units.map((unit) => `<a href="#${encodeURIComponent(section.section_id)}">${escapeHtml(unit.sub_2_id)} · ${escapeHtml(unit.sub_2_name)}</a>`).join("")}
    `;
  }
      ...
      els.grid.innerHTML = data.tracks.map((track) => {
        ...
        return `
          <section class="curv2-track-section">
            <h2 class="curv2-track-title">${escapeHtml(track.track_name)}</h2>
