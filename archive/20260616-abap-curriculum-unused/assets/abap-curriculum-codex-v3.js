(function () {
      ...
      <div class="curv2-premium-wrap">
        <section class="curv2-premium-hero">
          <div>
            ...
            <div class="curv2-kicker">ABAP Curriculum Codex v3</div>
  ...
  function renderToc(container, section) {
    ...
    container.innerHTML = `
      <h2>학습 목차</h2>
      <a href="#${encodeURIComponent(section.section_id)}" data-curv2-scroll-target="${escapeHtml(section.section_id)}">${escapeHtml(section.section_id)} · 개요</a>
      ${units.map((unit) => `<a href="#${encodeURIComponent(unit.sub_2_id)}" data-curv2-scroll-target="${escapeHtml(unit.sub_2_id)}">${escapeHtml(unit.sub_2_id)} · ${escapeHtml(unit.sub_2_name)}</a>`).join("")}
    `;
  }
