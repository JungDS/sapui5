    ...
    for tag in soup.find_all(True):
        if tag.get('class'):
            for c in tag.get('class'):
                if c.startswith('viz-') or c in ['interactive-data-chart', 'hotspot-explorer', 'hotspot']:
                    classes.add(c)
...
files = glob.glob(os.path.join(base_dir, "visuals", "*.html"))

print("=== Phase 2 (Visuals) 파일 콘텐츠 및 컴포넌트 분석 ===")
all_components = set()
for f in files:
    res = analyze_html(f)
    name = os.path.basename(f)
    issue_str = ", ".join(res['issues']) if res['issues'] else "문제 없음 (정상 데이터)"
    comp_str = ", ".join(res['components'])
    for c in res['components']:
        all_components.add(c)
    print(f"📄 {name:30} | 글자수: {res['text_len']:4} | 상태: {issue_str}")
    # print(f"   -> 컴포넌트: {comp_str}")
